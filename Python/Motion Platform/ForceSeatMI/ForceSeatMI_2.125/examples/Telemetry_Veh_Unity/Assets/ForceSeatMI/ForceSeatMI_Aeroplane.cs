...
namespace MotionSystems
{
	class ForceSeatMI_Aeroplane : ForceSeatMI_ITelemetryInterface
	{
		...
		public virtual void Update(float deltaTime, ref FSMI_TelemetryACE telemetry)
		{
			...
			{
				ForceSeatMI_Utils.LowPassFilter(ref telemetry.bodyLinearAcceleration[0].forward, (forwardSpeed - m_prevForwardSpeed) / deltaTime, FSMI_ACC_LOW_PASS_FACTOR);
				ForceSeatMI_Utils.LowPassFilter(ref telemetry.bodyLinearAcceleration[0].right,   (rightSpeed   - m_prevRightSpeed)   / deltaTime, FSMI_ACC_LOW_PASS_FACTOR);
				ForceSeatMI_Utils.LowPassFilter(ref telemetry.bodyLinearAcceleration[0].upward,  (upSpeed      - m_prevUpSpeed)      / deltaTime, FSMI_ACC_LOW_PASS_FACTOR);
				...
				ForceSeatMI_Utils.LowPassFilter(ref telemetry.bodyAngularVelocity[0].roll,  deltaAngles.z / deltaTime, FSMI_ANGLES_SPEED_LOW_PASS_FACTOR);
				ForceSeatMI_Utils.LowPassFilter(ref telemetry.bodyAngularVelocity[0].pitch, deltaAngles.x / deltaTime, FSMI_ANGLES_SPEED_LOW_PASS_FACTOR);
				ForceSeatMI_Utils.LowPassFilter(ref telemetry.bodyAngularVelocity[0].yaw,   deltaAngles.y / deltaTime, FSMI_ANGLES_SPEED_LOW_PASS_FACTOR);
			}
