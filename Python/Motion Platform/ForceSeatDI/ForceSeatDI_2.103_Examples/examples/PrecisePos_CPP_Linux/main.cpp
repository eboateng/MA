...
namespace
{
	...
	void send(const std::vector<FSDI_Handle>& apis, int index, const FSDI_TopTablePositionPhysical& pos)
	{
	...
	void work(std::vector<FSDI_Handle>& apis)
	{
		...
		pos.structSize = sizeof(FSDI_TopTablePositionPhysical);

		pos.mask = FSDI_BIT_PAUSE | FSDI_BIT_POSITION | FSDI_BIT_MAX_SPEED | FSDI_BIT_STRATEGY;
		pos.maxSpeed = 65535;
		pos.pause = FSDI_False;
		pos.strategy = FSDI_Strategy_FullMatch;
		...
		{
			// Prepare demo data
			auto value = sinf(iterator * 3.1415f / 180) * 15 * 3.1415f / 180;
			if (++iterator > 360)
			{
				iterator = 0;
			}
