...
namespace
{
	...
	const float DRAWING_PITCH_MAX = FMath::DegreesToRadians(10);

	// Maximum value of roll angle that is available in the game
	const float DRAWING_ROLL_MAX = FMath::DegreesToRadians(10);
}
...
void ATablePhyPos_UnrealPawn::Tick(float DeltaTime)
{
	...
	rotator.Roll     = FMath::RadiansToDegrees(CurrentDrawingRoll);
	rotator.Pitch    = FMath::RadiansToDegrees(CurrentDrawingPitch);
	...
	Board->SetRelativeLocationAndRotation(BoardLocation, rotator);
	...
	Shaft->SetRelativeLocation(ShaftLocation);
	...
	ShaftHead->SetRelativeLocation(ShaftHeadLocation);
