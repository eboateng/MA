...
#include "FastPos_UnrealPawn.h"
#include "FastPos_Unreal.h"
...
namespace
{
	...
	const int PLATFORM_POSITION_LOGIC_MIN = -32767;
	const int PLATFORM_POSITION_LOGIC_MAX = 32767;
...
AFastPos_UnrealPawn::AFastPos_UnrealPawn()
	...
	, IsLicenseValid(false)
{
	...
	PlatformPosition.mask     = FSDI_BIT_PAUSE | FSDI_BIT_POSITION | FSDI_BIT_MAX_SPEED;
	PlatformPosition.maxSpeed = 65535;
	PlatformPosition.pause    = FSDI_False;
	// ForceSeatDI - END
}

void AFastPos_UnrealPawn::Tick(float DeltaTime)
{
...
void AFastPos_UnrealPawn::SetupPlayerInputComponent(class UInputComponent* InputComponent)
{
	// Bind control axis' to callback functions
	InputComponent->BindAxis("MoveUp", this, &AFastPos_UnrealPawn::MoveUp);
	InputComponent->BindAxis("RotateForward", this, &AFastPos_UnrealPawn::RotateForward);
	InputComponent->BindAxis("RotateRight", this, &AFastPos_UnrealPawn::RotateRight);

	Super::SetupPlayerInputComponent(InputComponent);
}

void AFastPos_UnrealPawn::BeginPlay()
{
	Super::BeginPlay();
...
void AFastPos_UnrealPawn::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
void AFastPos_UnrealPawn::UpdateValue(float& value, float input, float step, float min, float max)
{
	if (0 < input)
	{
...
void AFastPos_UnrealPawn::SendCoordinatesToPlatform()
{
	// ForceSeatMI - BEGIN
	PlatformPosition.pause = FSDI_False;
	PlatformPosition.roll  = static_cast<FSDI_INT16>(FMath::Clamp<float>(CurrentDrawingRoll / DRAWING_ROLL_MAX  * PLATFORM_POSITION_LOGIC_MAX, PLATFORM_POSITION_LOGIC_MIN, PLATFORM_POSITION_LOGIC_MAX));
	PlatformPosition.pitch = static_cast<FSDI_INT16>(FMath::Clamp<float>(CurrentDrawingPitch / DRAWING_PITCH_MAX * PLATFORM_POSITION_LOGIC_MAX, PLATFORM_POSITION_LOGIC_MIN, PLATFORM_POSITION_LOGIC_MAX));
	PlatformPosition.heave = static_cast<FSDI_INT16>(FMath::Clamp<float>(CurrentDrawingHeave / DRAWING_HEAVE_MAX * PLATFORM_POSITION_LOGIC_MAX, PLATFORM_POSITION_LOGIC_MIN, PLATFORM_POSITION_LOGIC_MAX));
	
	IForceSeatDI::Get().SendTopTablePosLog(&PlatformPosition);
	// ForceSeatMI - END
}

void AFastPos_UnrealPawn::MoveUp(float Val)
{
	// Update drawing heave value with specified step
	UpdateValue(CurrentDrawingHeave, Val, DRAWING_HEAVE_STEP, 0, DRAWING_HEAVE_MAX);
}

void AFastPos_UnrealPawn::RotateForward(float Val)
{
...
void AFastPos_UnrealPawn::RotateRight(float Val)
{
