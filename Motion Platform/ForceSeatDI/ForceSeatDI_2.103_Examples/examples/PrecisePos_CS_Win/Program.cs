...
namespace TableFastPos_RuntimeLoading_CS
{
	class Program
	{
		...
		static void Send(List<ForceSeatDI> apis, int index, ref FSDI_TopTablePositionPhysical pos)
		{
			if (! apis[index].SendTopTablePosPhy(ref pos))
			{
		...
		ok = ok &&  ConnectUSB(ref apis, 0, null);
			...
			FSDI_TopTablePositionPhysical pos = new FSDI_TopTablePositionPhysical();
			pos.structSize = (byte)Marshal.SizeOf(pos);
			pos.mask = FSDI_BIT.PAUSE | FSDI_BIT.POSITION | FSDI_BIT.MAX_SPEED | FSDI_BIT.STRATEGY;
			pos.maxSpeed = 65535;
			pos.pause = 0;
			pos.strategy = (byte)FSDI_Strategy.FullMatch;
			...
			{
				// Prepare demo data
				float value = (float)Math.Sin(iterator * 3.1415f / 180) * 15 * 3.1415f / 180;
				if (++iterator > 360)
				{
					iterator = 0;
				}
